...
class DataLoader():
    def __init__(self, config, tokenizer=None, rank=0, world_size=1, hf_split="train", hf_config=None, streaming=False, small_data_size=None, cache = None, use_cache = False):
        ...
        hf_config: HuggingFace datasets の configuration name (例: "sample_10BT")
    ...
    def load_dataset(self, seed: int):
        ...
        if self.config.sub_target_files == "":
            # hf_configが指定されている場合は設定名として使用
            if self.hf_config:
                ds = load_dataset(path, self.hf_config, split=self.hf_split, streaming=False)
            else:
