...
class Rotary(nn.Module):
    def __init__(self, config):
        ...
        # RoPEの逆周波数（inv_freq）は学習パラメータではなく、設定から計算される固定値なので、register_buffer()を使用
...
class GroupedQueryAttention(nn.Module):
    ...
    def forward(self, x, cos, sin, start_pos = 0):
        ...
        if self.use_cache and c_context_len == 1:
            ...
            pos = torch.tensor([start_pos], device=x.device, dtype=inv_freq.dtype)
        ...
        if self.use_flash:
            ...
            # キャッシュ使用時（c_context_len == 1）は手動でアテンション計算を行う
            if self.use_cache and x.shape[1] == 1:
                # GQAの場合、keys/valuesをクエリヘッド数に手動展開する必要がある
                if keys.size(1) != queries.size(1):
                    # repeat_kv関数は[batch, seq_len, heads, dim]形式を期待するためtransposeする
...
class Transformer(nn.Module, PyTorchModelHubMixin): # extending PyTorchModelHubMixin for save weights as safetensors
    ...
    def forward(self, x: torch.Tensor, targets: Optional[torch.Tensor] = None, start_pos: int = 0):
        ...
        if targets is None:
            ...
            if self.use_moe and not self.use_lossfreebalance:
                loss = ce_loss + total_aux_loss    # in this case, ce_loss its loss w/o aux_loss
    ...
    def generate(self, x: torch.Tensor, max_tokens: int, temperature: float = 1.0, top_k: int = 50,
        ...
        for c_tkn_pos in range(max_tokens):
            if use_cache:
                if c_tkn_pos == 0:
                    # 最初は入力全体を処理
                    logits, _, ce_loss = self.forward(x, start_pos=0)
                else:
                    # start_posは実際のシーケンス位置を指定（初期長 + 生成済みトークン数 - 1）
                    actual_start_pos = initial_length + c_tkn_pos - 1
                    logits, _, ce_loss = self.forward(x[:, -1:], start_pos=actual_start_pos)
            else:
